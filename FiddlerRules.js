...
*/
    ...
    static function OnBeforeResponse(oSession: Session) {
        var basePath:String = "C:\\Users\\hong\\Desktop\\playground\\WeChat\\MiniProgram\\brain\\question.hortor.net\\question\\bat\\";
        var url:String = oSession.url.Split("?")[0];
        //MessageBox.Show(oSession.url + "   " + url);
        if (oSession.HostnameIs("question.hortor.net")) {
            if (url.EndsWith("match")) {
                MessageBox.Show(url);
                oSession.SaveResponseBody(basePath + "match");
            }
            else{
                if(url.EndsWith("findQuiz")) {
                    oSession.SaveResponseBody(basePath + "findQuiz");
                }
                else{
                    if(url.EndsWith("fightResult")) {
                        oSession.SaveResponseBody(basePath + "fightResult");
                    }
                    else{
                        if(url.EndsWith("choose")) {
                            oSession.SaveResponseBody(basePath + "choose");
                        }
						else{
							if(url.EndsWith("matchConfirm")){
								oSession.SaveResponseBody(basePath + "matchConfirm");
							}
						}
                    }
                }
